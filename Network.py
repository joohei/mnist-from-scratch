...
def create_batches(x, y, batch_size):
    num_batches = cp.int(cp.ceil(x.shape[0] / batch_size))
    x_train = cp.array_split(x, num_batches)
    y_train = cp.array_split(y, num_batches)
    ...
    def train(self, data_set, test_set, learn_rate=0.01, momentum=0.9, clip_value=1.0, epochs=1000, verbose=True):
        ...
        with SummaryWriter(f"logs/{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}") as logger:
            ...
            for epoch in range(epochs):
                ...
                if verbose:
                    logger.add_scalar("loss/training", cp.asnumpy(self.cost(data_set)), global_step=epoch)
                    logger.add_scalar("loss/validation", cp.asnumpy(self.cost(test_set)), global_step=epoch)
                    logger.add_scalar("accuracy/training", cp.asnumpy(self.accuracy(data_set)), global_step=epoch)
                    logger.add_scalar("accuracy/validation", cp.asnumpy(self.accuracy(test_set)), global_step=epoch)

    def accuracy(self, data_set):
        return cp.mean(self.classify(data_set.x) == self.labelify(data_set.y))
